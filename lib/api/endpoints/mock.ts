...
export const mockApi = {
  ...
  cjAuthorize: async (input: Record<string, unknown>) => { await pause(); return mockOk({ ...input, authorized: true }); },
  ...
  impactAuthorize: async (input: Record<string, unknown>) => { await pause(); return mockOk({ ...input, authorized: true }); },
  ...
  impactTrackingLink: async (input: Record<string, unknown>) => { await pause(); return mockOk({ shortUrl: "https://trk.local/abc", request: input }); },
  wpAuthorize: async (input: Record<string, unknown>) => { await pause(); return mockOk({ ...input, authorized: true }); },
  wpUploadMedia: async (filename: string) => { await pause(); return mockOk({ id: "media-1", filename }); },
  wpCreateProduct: async (input: Record<string, unknown>) => { await pause(); return mockOk({ id: "wp-prod-1", ...input }); },
  ...
  metricoolUploadMedia: async (input: Record<string, unknown>) => { await pause(); return mockOk({ id: "met-media-1", ...input }); },
  metricoolCreatePost: async (input: Record<string, unknown>) => { await pause(); return mockOk({ id: "sched-1", ...input }); },
